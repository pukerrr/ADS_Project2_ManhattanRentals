...
timeSeries <- function(zipcode)
{
  ...
  check <- sum(ifelse(selected_zip %in% median_price$RegionName, 1,0))
  
  if(length(selected_zip) == check)
  {
    ...
    new_time_series$date.new <- as.Date(paste0(new_time_series$date,"-01"),"%Y-%m-%d")
